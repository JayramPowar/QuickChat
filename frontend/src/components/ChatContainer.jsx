...
const ChatContainer = () => {
  ...
  //* used to handle the auto scroll to bottom when new message arrives
  useEffect(() => {
  ...
  return selectedUser ? (
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
      ...
      <div className="flex items-center gap-3 py-3 mx-4 border-b border-stone-500">
        ...
        <p className="flex-1 text-lg text-white flex items-center  gap-2">
          ...
          {onlineUser.includes(selectedUser._id) && <span className="w-2 h-2 bg-green-400 rounded-full"></span>}
        </p>
        <img
      ...
      <div className="absolute bottom-0 left-0 right-0 flex items-center gap-2 p-3 ">
        <div className="flex-1 flex items-center rounded-full bg-gray-100/12 px-4">
          <input onChange={(e)=>setInput(e.target.value)} value={input} onKeyDown={(e)=>e.key == "Enter" ? handleSendMessage(e) : null} type="text" placeholder="Send message here" className="flex-1 text-sm p-3 border-none rounded-lg outline-none text-white placeholder-gray-400"/>
    ...
    <div className="flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden">
      <img src={assets.logo_icon} className="max-w-15" alt="" />
